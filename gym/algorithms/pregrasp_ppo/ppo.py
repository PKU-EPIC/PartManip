...
class PREGRASP_PPO:
    def __init__(self,
        ...
        if self.is_testing and model_cfg["save_obs_path"] is not None:
            ...
            self.vec_env.task.cfg["test_save_video"] = True
            self.test_obs = []
            self.use_pc = True
        else:
            self.test_save = False
            self.vec_env.task.cfg["test_save_video"] = False
    ...
    def pre_grasp(self):
        ...
        #     self.vec_env.task.initial_dof_states = torch.load("/data2/pregrasp_{}.pth".format(self.vec_env.task.cfg["task"]["target_part_category"]), map_location=self.device)
        #     return
        # elif "fixed" not in self.vec_env.task.cfg["env"]["robotName"]:
        #     return
        # else:
