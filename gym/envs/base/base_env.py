...
# Env for any robot, asset and representation. 
# only focus on loading sim, viewer, some buffers and setting randomization
class BaseEnv():
    def __init__(self, cfg, enable_camera_sensors=False):
        ...
        print("enable_camera_sensors", enable_camera_sensors)
        if enable_camera_sensors == False and self.headless == True:
            self.graphics_device_id = -1

        self.num_envs = self.env_num # TODO
        self.num_obs = self.obs_dim
        self.num_states = self.state_dim

        # self.num_states = cfg["env"].get("numStates", 0)
        self.num_actions = self.num_actions # TODO
    ...
    def step(self, actions):
        # if self.dr_randomization.get("actions", None):
        #     actions = self.dr_randomization["actions"]["noise_lambda"](actions)
        ...
        if self.dr_randomization.get("observations", None):
            self.obs_buf = self.dr_randomization["observation"]["noise_lambda"](self.obs_buf)
    ...
    # TODO RL Domain Randomization in our setting    
