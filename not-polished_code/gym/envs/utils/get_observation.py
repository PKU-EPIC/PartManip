...
def _get_pc_observation(task, suggested_gt=None, use_pc = True):
    task.gym.refresh_actor_root_state_tensor(task.sim)
    task.gym.refresh_dof_state_tensor(task.sim)
    task.gym.refresh_rigid_body_state_tensor(task.sim)
    if "ik" in task.cfg["env"]["driveMode"]:
    ...
    obs__ = _get_base_observation(task, suggested_gt)
    # import pdb
    # pdb.set_trace()
    if task.cfg["test_save_video"]:
        depth_bar = task.cfg["obs"]["pc"]["depth_bar"]
        pc, img = task.compute_point_cloud_state(depth_bar, return_img=True)
        obs__.points = pc
        obs__.imgs = img
        task.obs_buf = obs__
        return obs__
    ...
    if use_pc:
        depth_bar = task.cfg["obs"]["pc"]["depth_bar"]
        pc = task.compute_point_cloud_state(depth_bar)
        obs__.points = pc
    task.obs_buf = obs__
    return obs__

def _refresh_pc_observation(task, suggested_gt=None, use_pc = True):
    return _get_pc_observation(task, suggested_gt, use_pc)