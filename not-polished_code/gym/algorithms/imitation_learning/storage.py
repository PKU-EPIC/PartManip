...
class RolloutStoragePC_feature:

    def __init__(self, num_envs, num_transitions_per_env, obs_shape, states_shape, actions_shape, point_cloud_feature_shape, device='cpu', sampler='sequential'):
    ...
    def compute_returns(self, last_values, gamma, lam):
        ...
        for step in reversed(range(self.num_transitions_per_env)):
            if step == self.num_transitions_per_env - 1:
                ...
                next_values = self.values[step + 1]
            next_is_not_terminal = 1.0 - self.dones[step].float()
            delta = self.rewards.detach()[step] + next_is_not_terminal * gamma * next_values - self.values[step]
            advantage = delta + next_is_not_terminal * gamma * lam * advantage
            self.returns[step] = advantage + self.values[step]
...
class RolloutStorage_expert:
    ...
    def add_transitions(self, observations, actions, expert_actions):
        if self.step >= self.num_transitions_per_env:
            raise AssertionError("Rollout buffer overflow")
