...
class imitation_learning:
    ...
    def test(self, path):
        ...
        # print(checkpoint_dict["model_state_dict"].keys())
    ...
    def load_expert(self, path):
        ...
        if self.use_expert:
            ...
            for i in range(self.train_env_num // self.vec_env.task.env_per_asset):
                ...
                dict = np.load(path_i_new, map_location=self.device)

                assert(self.demo_num <= dict["traj_num"])
                self.demo_obs[:, i] = torch.from_numpy(dict["observations"][:self.demo_num],device=self.device)
                self.demo_actions[:, i] = torch.from_numpy(dict["actions"][:self.demo_num], device=self.device)
                
                if self.debug:
                    # np.savetxt("/data2/ziming/exp_new/test_obs/0_franka.txt", self.demo_obs[0, 0, :, self.qpose_num:self.qpose_num+13].cpu().numpy())
                    # np.savetxt("/data2/ziming/exp_new/test_obs/0_handle.txt", self.demo_obs[0, 0, :, self.qpose_num+16:self.qpose_num+28].cpu().numpy())
                    # np.savetxt("/data2/ziming/exp_new/test_obs/0_part.txt", self.demo_obs[0, 0, :, self.qpose_num+28:self.qpose_num+40].cpu().numpy())
                    # exit()    
                    continue
                
                pcs = torch.from_numpy(dict["pcs"][:self.demo_num, :, :, :6], device=self.device)
                
                if self.backbone_freeze:
                    self.demo_pcs[:, i] = self.actor_critic.get_features(pcs.reshape(-1, pcs.shape[-2], pcs.shape[-1])).reshape(self.demo_num, self.num_transitions_per_env, -1)
                else:
                    self.demo_pcs[:, i] = pcs
    ...
    def uncanonicalize(self, actions):
        ...
        #print(actions.shape)
        #print(self.quaternion_rot_invert.shape)
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for i in range(self.max_episode_length): 
                    ...
                    if self.valIntra_env_num or self.valInter_env_num :
                        ...
                        current_valIntra_states = current_states[self.train_env_num:self.valIntra_env_num + self.train_env_num]
                        current_valInter_states = current_states[self.valIntra_env_num + self.train_env_num:]
                        current_train_obs_new = Observations(state=current_train_obs.state[:, self.qpose_num:self.qpose_num+16], points=current_train_obs.points)
                        current_valIntra_obs_new = Observations(state=current_valIntra_obs.state[:, self.qpose_num:self.qpose_num+16], points=current_valIntra_obs.points)
                        current_valInter_obs_new = Observations(state=current_valInter_obs.state[:, self.qpose_num:self.qpose_num+16], points=current_valInter_obs.points)
                ...
                ##############test_demo_obs############
                #print(torch.mean(self.demo_obs[0, 1, :, self.qpose_num:self.qpose_num+16] -    self.storage.observations[:, 1, self.qpose_num:self.qpose_num+16], dim=0))
                #print(torch.mean(self.demo_obs[0, 1, :, self.qpose_num+16:self.qpose_num+28] - self.storage.observations[:, 1, self.qpose_num+16:self.qpose_num+28], dim=0))
                #print(torch.mean(self.demo_obs[0, 1, :, self.qpose_num+28:self.qpose_num+40] - self.storage.observations[:, 1, self.qpose_num+28:self.qpose_num+40], dim=0))
                #
                mean_trajectory_length, mean_reward = self.storage.get_statistics()
    ...
    def discriminator_update(self, it):
        ...
        obs_batch = self.storage.observations.reshape(-1, self.storage.observations.size(-1))
        ...
        if self.use_expert:
            expert_actions_batch = self.expert_storage.expert_actions.permute(1, 0, 2).reshape(-1, self.expert_storage.actions.size(-1))
            expert_obs_batch = obs_batch
            expert_pcs_batch = pcs_batch
        else:
            rand_num = random.randint(0, self.demo_num - 1)
            expert_obs_batch = self.demo_obs[rand_num].reshape(-1, self.storage.observations.size(-1))
            expert_actions_batch = self.demo_actions[rand_num].reshape(-1, self.storage.actions.size(-1))
            # print(self.demo_pcs.shape)
            # print(self.storage.pointcloud.size(-2), self.storage.pointcloud_features.size(-1))
            if not self.debug:
                if self.backbone_freeze:
                    expert_pcs_batch = self.demo_pcs[rand_num].reshape(-1, self.demo_pcs[rand_num].size(-1))
                else:
                    expert_pcs_batch = self.demo_pcs[rand_num].reshape(-1, self.demo_pcs[rand_num].size(-2), self.demo_pcs[rand_num].size(-1))
