...
class DAGGER_PPO(PREGRASP_PPO):
    def __init__(self,
        ...
        self.backbone_freeze = cfg_algo["policy"]["freeze"]
        self.pc_feature_shape = cfg_algo["policy"]["feature_dim"]
        self.debug = cfg_algo["policy"]["GAIL_debug"]
        ...
        self.use_expert = cfg_algo["policy"]["use_expert"] # use expert policy network or use demostration
        ...
        self.use_self_imitation = cfg_algo["policy"]["use_self_imitation"]
        self.expert_buffer_size = expert_cfg["expert_buffer_size"]
        self.use_seg = self.cfg_algo["policy"]["use_seg"]
        self.use_domain_discriminator = self.cfg_algo["policy"]["use_domain_discriminator"]
        ...
        if not self.is_testing:
            ...
            if self.use_expert:
                self.expert_storage = RolloutStorage_expert(self.train_env_num, self.expert_buffer_size, self.observation_space.shape, self.pc_space.shape,
                                        self.state_space.shape, self.action_space.shape, self.device, sampler, use_seg = self.cfg_algo["policy"]["use_seg"])
            if self.cfg_algo["policy"]["use_bc_pretrain"]:
                self.optimizer_bc = torch.optim.Adam(self.actor_critic.parameters(), lr=learning_rate)
                self.loss_function_bc = torch.nn.MSELoss()
            #     self.demo_num = cfg_algo["policy"]["demo_num"]
            ...
            self.optimizer_dagger = optim.Adam(self.actor_critic.parameters(), lr=cfg_algo["policy"]["learning_rate_dagger"])   # , weight_decay=float(self.weight_decay), trick 9
                ...
                for r in range(self.eval_round) :
                    ...
                    for i in range(self.max_episode_length):
                        ...
                        if self.use_expert:
                            ...
                            expert_actions, _ = self.actor_critic_expert.act_inference(current_obs_expert, concat_part_center = self.cfg_algo["policy"]["concat_part_center"])
            ...
            with torch.no_grad() :
                for r in range(self.eval_round) :
                    for i in range(self.max_episode_length):
                        ...
                        actions, _ = self.actor_critic.act_inference(current_obs, concat_part_center = self.cfg_algo["policy"]["concat_part_center"])
        ...
        for it in range(self.current_learning_iteration, num_learning_iterations):
            ...
            for i in range(self.max_episode_length): 
                ...
                if not self.use_pc:
                    ...
                    # Compute the action
                train_actions, train_actions_log_prob, train_values, train_mu, train_sigma, others = self.actor_critic.act(current_train_obs, require_grad = False, concat_part_center = self.cfg_algo["policy"]["concat_part_center"])
                actions = train_actions
                # train_actor_actions, _ , _ = self.actor_critic.act_dagger(current_train_obs)
                
                if self.valIntra_env_num:
                    valIntra_actions, others = self.actor_critic.act_inference(current_valIntra_obs, concat_part_center = self.cfg_algo["policy"]["concat_part_center"])
                ...
                if self.valInter_env_num:
                    valInter_actions, others = self.actor_critic.act_inference(current_valInter_obs, concat_part_center = self.cfg_algo["policy"]["concat_part_center"])
                ...
                if (i + 1) % self.num_transitions_per_env == 0 and (i + 1) != self.max_episode_length:
                    ...
                    _, _, last_values, _, _, _ = self.actor_critic.act(current_obs, concat_part_center = self.cfg_algo["policy"]["concat_part_center"])
            ...
            _, _, last_values, _, _ , _= self.actor_critic.act(current_obs, current_states, concat_part_center = self.cfg_algo["policy"]["concat_part_center"])
            stop = time.time()
            collection_time = stop - start
    ...
    def load_expert(self, path, demo_path = None):
        ...
        if self.cfg_algo["policy"]["use_bc_pretrain"] and demo_path is not None:
            self.demo_num = self.cfg_algo["policy"]["demo_num"]
            ...
            print("loading demo")
            path = os.path.join(demo_path, '{}'.format(self.vec_env.task.cfg["task"]["target_part_category"]))
            i = 0
            for _ in range(self.train_env_num // self.vec_env.task.env_per_asset):
                path_i_new = path + '/{}.npy'.format(str(self.vec_env.task.selected_asset_path_list[_]).split("/")[-1])
                if os.path.exists(path_i_new):
                    print("load demo_{} from ".format(i), path_i_new.split("/")[-1])
                    ...
                    p = path_i_new.split("/")[-1]
                    print(f"{_}: {p} is not found !")
    ...
    def pretrain_bc(self):
        ...
        for ep in tqdm(range(self.cfg_algo["policy"]["bc_epochs"])):
            l = int(num_samples / self.cfg_algo["policy"]["mb_size"])
            for mb in range(l):
                rand_idx = torch.from_numpy(np.random.choice(num_samples, size=self.cfg_algo["policy"]["mb_size"]))
                ...
                if self.cfg_algo["policy"]["use_dagger_activation"]:
                    if self.cfg_algo["policy"]["action_normalization"] == "tanh":
                        expert_actions_tensor = torch.tanh(act) # torch.clamp(expert_actions_batch, -self.vec_env.clip_actions, self.vec_env.clip_actions) # expert_actions_batch # 
                        actions_tensor = torch.tanh(act_pred) # torch.clamp(actions_batch, -self.vec_env.clip_actions, self.vec_env.clip_actions)
                    elif self.cfg_algo["policy"]["action_normalization"] == "clip":
                        expert_actions_tensor = torch.clamp(act, -self.vec_env.clip_actions, self.vec_env.clip_actions) # expert_actions_batch # 
                        actions_tensor = torch.clamp(act_pred, -self.vec_env.clip_actions, self.vec_env.clip_actions)
                    else:
                        expert_actions_tensor = act # torch.clamp(expert_actions_batch, -self.vec_env.clip_actions, self.vec_env.clip_actions) # expert_actions_batch # 
                        actions_tensor = act_pred # torch.clamp(actions_batch, -self.vec_env.clip_actions, self.vec_env.clip_actions)
                else:
                    expert_actions_tensor = act # torch.clamp(expert_actions_batch, -self.vec_env.clip_actions, self.vec_env.clip_actions) # expert_actions_batch # 
                    actions_tensor = act_pred
                self.optimizer_bc.zero_grad()
                loss = self.loss_function_bc(expert_actions_tensor, actions_tensor)
                loss.backward()
    ...
    def update(self, it):
        ...
        for epoch in range(self.num_learning_epochs):
            ...
            for indices, indices_expert in zip(batch, batch_expert):
                ...
                actions_batch, _ , others = self.actor_critic.act_dagger(obs_now_expert, concat_part_center = self.cfg_algo["policy"]["concat_part_center"]) #self.expert_storage.actions.view(-1, self.storage.actions.size(-1))[indices]
                ...
                # surrogate_loss = F.mse_loss(actions_batch, expert_actions_batch)  # self.l2(actions_batch, expert_actions_batch)/ actions_batch.shape[0] * self.cfg_algo["policy"]["dagger_loss_supervise"]
                if self.use_seg:
                    seg_loss_proportion = self.cfg_algo["policy"]["dagger_loss_seg"]
                else:
                    seg_loss_proportion = 0.
                
                dagger_loss_proportion = self.cfg_algo["policy"]["dagger_loss_supervise"] * max(0,(1-it*self.cfg_algo["policy"]["dagger_loss_decay_proportion"]))  # self.l2(actions_batch, expert_actions_batch)/ actions_batch.shape[0] * self.cfg_algo["policy"]["dagger_loss_supervise"]
                ...
                if self.use_seg:
                    ...
                    if self.cfg_algo["policy"]["seg_focal_loss"]:
                ...
                if self.cfg_algo["policy"]["joint_update"] and it % self.cfg_algo["policy"]["update_pn_interval"] != 0:
                    if self.use_seg:
                        seg_loss.backward()
                    self.optimizer_dagger.step()
                elif self.cfg_algo["policy"]["joint_update"] : #只update backbone
                    if self.cfg_algo["policy"]["update_which"] == "backbone":
                ...
                actions_log_prob_batch, entropy_batch, value_batch, mu_batch, sigma_batch = \
                    self.actor_critic.evaluate(observations, actions_batch, self.cfg_algo["policy"]["actor_freeze"] ,self.cfg_algo["policy"]["rl_backbone_freeze"], concat_part_center = self.cfg_algo["policy"]["concat_part_center"])
                ...
                rl_loss_proportion = self.cfg_algo["policy"]["dagger_loss_RL"]
