            ...
            if not self.debug:
                if self.backbone_freeze:
                    self.storage = RolloutStoragePC_feature(self.train_env_num, num_transitions_per_env, self.observation_space.shape,
                                            self.state_space.shape, self.action_space.shape, self.pc_feature_shape, self.device, sampler, use_imit=True, max_length=self.max_episode_length)
                else:
                    self.storage = RolloutStoragePC(self.train_env_num, num_transitions_per_env, self.observation_space.shape,
                                        self.state_space.shape, self.action_space.shape, self.pc_space.shape, self.device, sampler, use_imit=True, max_length=self.max_episode_length)
            else:
                self.storage = RolloutStorage(self.train_env_num, num_transitions_per_env, self.observation_space.shape,
                    self.state_space.shape, self.action_space.shape, self.device, sampler, use_imit=True, max_length=self.max_episode_length)
        ...
        if self.is_testing:
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for i in range(self.max_episode_length): 
                    ...
                    # else:
                    if not self.debug:
                        points_train = current_obs.points[:self.train_env_num]
                        points_valIntra = current_obs.points[self.train_env_num:self.valIntra_env_num + self.train_env_num]
                        points_valInter = current_obs.points[self.valIntra_env_num + self.train_env_num:]
                    else:
                        points_train = None
                        points_valIntra = None
                        points_valInter = None
                    current_train_obs = Observations(state=current_obs.state[:self.train_env_num], points= points_train, obs=current_obs.obs[:self.train_env_num])
                    if self.valIntra_env_num:
                        current_valIntra_obs = Observations(state=current_obs.state[self.train_env_num:self.valIntra_env_num + self.train_env_num],obs=current_obs.obs[self.train_env_num:self.valIntra_env_num + self.train_env_num], points= points_valIntra)
                        # current_valIntra_states = current_states[self.train_env_num:self.valIntra_env_num + self.train_env_num]
                    if self.valIntra_env_num:
                        current_valInter_obs = Observations(state=current_obs.state[self.valIntra_env_num + self.train_env_num:],obs=current_obs.obs[self.valIntra_env_num + self.train_env_num:], points= points_valInter)
                        # current_valInter_states = current_states[self.valIntra_env_num + self.train_env_num:]
                    
                    #####use noqpose and no boundingbox observations for actor

                    # Compute the action
                    train_actions, train_actions_log_prob, train_values, train_mu, train_sigma , others = self.actor_critic.act(current_train_obs)
                    actions = train_actions
                    train_features = features = others["feature"]
                    ...
                    train_rews = rews[:self.train_env_num] * self.model_cfg["disc_reward_perportion"] * torch.clip(torch.tensor(it / 100), max=1, min=0) + gt_rews[:self.train_env_num]
                    ...
                    if (i + 1) % self.num_transitions_per_env == 0 and (i + 1) != self.max_episode_length:
                        ...
                        disc_loss = self.discriminator_update(it, update=False)
                ...
                if self.use_self_imitation:
                    assert(self.storage.imit_step == 0)
                    for i in range(self.train_asset_num):
                        for j in range(self.vec_env.task.env_per_asset):
                            mean_reward_traj = torch.mean(self.storage.rewards_imit[:, i*self.vec_env.task.env_per_asset+j], dim=0)
                            if total_success[i*self.vec_env.task.env_per_asset+j] and mean_reward > 0:
                                # import pdb
                                # pdb.set_trace()
                                self.demo_states[self.demo_step[i], i] = self.storage.states_imit[:, i]
                                self.demo_actions[self.demo_step[i], i] = self.storage.actions_imit[:, i]
                                if not self.debug:
                                    if self.backbone_freeze:
                                        self.demo_pcs[self.demo_step[i], i] = self.storage.pointcloud_features_imit[:, i]
                                    else:
                                        self.demo_pcs[self.demo_step[i], i] = self.storage.pointcloud_imit[:, i]
