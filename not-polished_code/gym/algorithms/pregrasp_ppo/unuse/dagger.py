...
class DAGGER_PPO:
    def __init__(self,
        ...
        if not self.is_testing:
            ...
            if self.use_expert:
                self.expert_storage = RolloutStorage_expert(self.train_env_num, self.expert_buffer_size, self.state_space.shape,
    ...
    def load(self, path):
        ...
        try:
            ...
            self.optimizer.load_state_dict(checkpoint_dict["optimizer_state_dict"], strict = False)
            self.optimizer_dagger.load_state_dict(checkpoint_dict["optimizer_dagger_state_dict"], strict = False)
            self.discriminator.load_state_dict(checkpoint_dict["discriminator_dict"], strict = False)
            self.disc_optimizer.load_state_dict(checkpoint_dict["discriminator_dict"], strict = False)
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for i in range(self.max_episode_length): 
                    ...
                    if (i + 1) % self.num_transitions_per_env == 0 and (i + 1) != self.max_episode_length:
                        ...
                        mean_value_loss, mean_surrogate_loss, mean_dagger_loss = self.update(it)
                        self.storage.clear()
                        self.expert_storage.clear()
                        torch.cuda.synchronize()
                        stop = time.time()
    ...
    def update(self, it):
        ...
        for epoch in range(self.num_learning_epochs):
            ...
            for indices in batch:
                ...
                actions_batch, _ , _ = self.actor_critic.act_dagger(obs_now) #self.expert_storage.actions.view(-1, self.storage.actions.size(-1))[indices]
                # actions_batch_new = actions_batch.clone()
                # expert_actions_batch_new = expert_actions_batch.clone()
                # self.clip_actions = 3.
                ...
                dagger_loss = F.mse_loss(actions_tensor, expert_actions_tensor)  # self.l2(actions_batch, expert_actions_batch)/ actions_batch.shape[0] * self.model_cfg["dagger_loss_supervise"]
                ...
                surrogate_clipped = -torch.squeeze(advantages_batch) * torch.clamp(ratio, 1.0 - self.clip_param,
                                                                                1.0 + self.clip_param)
                surrogate_loss = torch.max(surrogate, surrogate_clipped).mean() * self.model_cfg["dagger_loss_RL"]
        ...
        # del loss, actions_batch,actions_batch_new, expert_actions_batch, expert_actions_batch_new
        ...
        return mean_value_loss, mean_surrogate_loss, mean_dagger_loss
