...
class ActorCritic(nn.Module):
    ...
    def __init__(self, obs_shape, states_shape, actions_shape, initial_std, model_cfg, asymmetric=False, qpose_num = 0, device=None):
        ...
        # Policy
        actor_layers = []
        actor_layers.append(nn.Linear(obs_shape[0], actor_hidden_dim[0]))
        ...
        if self.asymmetric:
            ...
            critic_layers.append(nn.Linear(obs_shape[0], critic_hidden_dim[0]))
    ...
    def act(self, observations, states):
        #observations = observations[:, self.qpose_num:]
...
#         return y2
class MyNet(nn.Module):
    def __init__(self, point_net, fc):
        super(MyNet, self).__init__()
        self.point_net = point_net
        self.fc = fc
        # self.split = split
...
class MyNet_pretrain(nn.Module):
    def __init__(self, backbone, fc):
        super(MyNet_pretrain, self).__init__()
        self.point_net = backbone
        self.fc = fc
        # self.split = split
    ...
    def forward(self, input):
        ...
        # pdb.set_trace()
        input_pc = input["pc"]
        device = self.point_net.device
        # print(device)
        pcs = []
        for points in input_pc:
            pc = PointCloud(scene_id=["train"],points=points[:,:6].cpu(),)
...
class ActorCriticPC(nn.Module):
    ...
    def __init__(self, obs_shape, states_shape, actions_shape, initial_std, model_cfg, asymmetric=False, qpose_num = 0, device = None):
        ...
        if self.use_spconv:
            self.backbone = _load_perception_model(model_cfg["Spconv"], device = self.device)
        ...
        if self.use_spconv:
            self.actor = MyNet_pretrain(self.backbone, self.actor1)
            self.critic = MyNet_pretrain(self.backbone, self.critic1)
        else:
            self.actor = MyNet(self.pointnet_layer, self.actor1)
            self.critic = MyNet(self.pointnet_layer, self.critic1)
